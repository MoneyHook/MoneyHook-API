...
func (h *Handler) respondV1TransactionStoreError(c echo.Context, err error) error {
	...
	case errors.Is(err, transactiondomain.ErrInvalidRelation):
		return httpx.RespondV1Error(c, http.StatusNotFound, "RELATED_RESOURCE_NOT_FOUND", "利用可能なカテゴリ、サブカテゴリ、または支払方法が見つかりません", nil)
	default:
		return httpx.RespondV1Error(c, http.StatusInternalServerError, "INTERNAL_ERROR", "サーバーエラーが発生しました", nil)
	}
}
