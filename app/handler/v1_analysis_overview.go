...
func (h *Handler) getV1AnalyticsOverview(c echo.Context) error {
	...
	if err != nil {
		return respondV1Error(c, http.StatusInternalServerError, "INTERNAL_ERROR", "分析データの取得に失敗しました", nil)
	}
	...
	if query.Compare == "previous_period" {
		...
		if err != nil {
			return respondV1Error(c, http.StatusInternalServerError, "INTERNAL_ERROR", "比較データの取得に失敗しました", nil)
		}
	}
