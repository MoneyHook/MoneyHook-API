...
API起動時には、PostgreSQLの接続・DB作成後にスキーママイグレーションが実行されます。必須のマスターデータは毎回整合されます。
...
Compose起動時の処理順は、Firebase Authの初期化、開発ユーザーのprovision、PostgreSQLのマイグレーション・シード、API起動です。`ENABLE_DEVELOPMENT_USER=true` かつ `ENABLE_SEED_DATA=true` の場合に限り、固定UID `a77a6e94-6aa2-47ea-87dd-129f580fb669` の開発用Googleユーザーがprovisionされます。これらの開発用フラグは本番環境では有効にしないでください。
